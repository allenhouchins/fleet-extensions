...
# macOS NuGet Packages extension installer script
# Downloads and installs the latest nuget_packages.ext from GitHub
...
EXTENSION_NAME="nuget_packages.ext"
...
echo "Starting NuGet Packages Extension installation..."
...
remove_old_extension() {
    local old_extension_name="nuget_packages.ext"
...
main() {
    ...
    log "=== NuGet Packages Extension Installer Started ==="
    if [[ "$IMMEDIATE_RESTART" == "immediate" ]]; then
