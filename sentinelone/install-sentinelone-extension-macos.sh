...
# SentinelOne extension installer script for macOS
# Downloads and installs the correct sentinelone extension from GitHub
# Supports amd64 (Intel) and arm64 (Apple Silicon) architectures
#
# Usage:
#   sudo ./install-sentinelone-extension-macos.sh
...
BACKUP_PATH=""
SENTINEL_CTL_PATHS=("/usr/local/bin/sentinelctl" "/opt/sentinelone/bin/sentinelctl")

echo "Starting SentinelOne Extension installation for macOS..."
...
check_macos() {
    log "Checking if running on macOS..."

    if [[ "$(uname -s)" == "Darwin" ]]; then
        local version
        version=$(sw_vers -productVersion)
        log "macOS detected: $(sw_vers -productName) $version"
        return 0
    else
        log "Error: This script is designed for macOS. Detected: $(uname -s)"
        exit 1
    fi
}
...
detect_architecture() {
    ...
    case "$arch" in
        "x86_64")
            EXTENSION_NAME="sentinelone-darwin-amd64.ext"
            log "Architecture detected: amd64 (Intel x86_64)"
            ;;
        "arm64")
            EXTENSION_NAME="sentinelone-darwin-arm64.ext"
            log "Architecture detected: arm64 (Apple Silicon)"
            ;;
        *)
            log "Error: Unsupported architecture: $arch"
            log "This script supports amd64 (x86_64) and arm64 only"
            exit 1
            ;;
    esac
...
check_prerequisites() {
    ...
    if ! command -v curl &> /dev/null; then
        ...
        log "Please install curl using Homebrew: brew install curl"
        log "Or install Xcode Command Line Tools: xcode-select --install"
        exit 1
    else
        log "curl is installed"
    fi

    # Check if sentinelctl is available (warning only, not required for extension to work)
    local sentinelctl_found=false
    for sentinelctl_path in "${SENTINEL_CTL_PATHS[@]}"; do
        if [[ -x "$sentinelctl_path" ]]; then
            log "SentinelOne CLI found at: $sentinelctl_path"
            sentinelctl_found=true
            break
        fi
    done

    if [[ "$sentinelctl_found" == false ]]; then
        log "WARNING: SentinelOne CLI (sentinelctl) not found at expected locations:"
        for path in "${SENTINEL_CTL_PATHS[@]}"; do
            log "  - $path"
        done
        log "The extension will still be installed, but it requires SentinelOne agent to function."
        log "If SentinelOne is installed, the extension will work once the agent is running."
        log ""
        log "Continuing with installation in 5 seconds... (Ctrl+C to cancel)"
        sleep 5
    fi
...
restart_orbit_service() {
    ...
    # Check if orbit service is running (using launchctl for macOS)
    if launchctl list | grep -q "com.fleetdm.orbit"; then
        ...
        if launchctl kickstart -k system/com.fleetdm.orbit; then
            ...
            log "Try manually: sudo launchctl kickstart -k system/com.fleetdm.orbit"
            return 1
        fi
    else
...
# Main execution
main() {
    log "=== SentinelOne Extension Installer for macOS Started ==="
    ...
    echo ""
    log "Note: The extension requires SentinelOne agent to be installed and running."
    log "      Query the table with: SELECT * FROM sentinelone_info;"
}
